...
def get_rings(edge_index, max_k=7):
    """
    Find induced (chordless) simple cycles of length 3..max_k in an undirected graph.

    Returns:
        rings: List[Tuple[int, ...]] where each tuple is an ordered cycle (node ordering).
               Different rotations / reverse orderings are deduplicated.
    """
    ...
    def is_induced_cycle(cycle):
        """cycle is a list of distinct nodes in cyclic order."""
        k = len(cycle)
        S = set(cycle)
        H = G.subgraph(S)

        # In an induced k-cycle: edges among S are exactly k and every node has degree 2 within S
        if H.number_of_edges() != k:
            return False
        return all(H.degree(v) == 2 for v in S)

    def canonicalize_cycle(cycle):
        """
        Canonical representation up to rotation and reversal, while keeping an order to use as boundary.
        Returns a tuple in chosen canonical cyclic order.
        """
        cycle = list(cycle)
        k = len(cycle)

        # rotate so the smallest node id comes first
        min_pos = min(range(k), key=lambda i: cycle[i])
        rot1 = cycle[min_pos:] + cycle[:min_pos]

        # do the same for reversed direction
        rev = list(reversed(cycle))
        min_pos_r = min(range(k), key=lambda i: rev[i])
        rot2 = rev[min_pos_r:] + rev[:min_pos_r]

        # pick lexicographically smaller
        return tuple(rot1) if tuple(rot1) <= tuple(rot2) else tuple(rot2)

    rings = []
    seen = set()

    # Enumerate simple cycles up to max_k via bounded DFS.
    # This is OK for small max_k (<=7) typical in molecular/tu datasets.
    nodes = list(G.nodes())
    for start in nodes:
        stack = [(start, [start])]
        while stack:
            ...
            for nbr in G.neighbors(v):
                if nbr == start and len(path) >= 3:
                    cycle = path[:]  # closes back to start
                    if 3 <= len(cycle) <= max_k and is_induced_cycle(cycle):
                        key = tuple(sorted(cycle))
                        if key not in seen:
                            seen.add(key)
                            rings.append(canonicalize_cycle(cycle))
                    continue

                # enforce an ordering to reduce duplicates early:
                # only grow paths where nodes are >= start (keeps cycles discovered once-ish)
                if nbr in path:
                    continue
                if nbr < start:
                    continue
...
def compute_ring_2complex(x: Union[Tensor, np.ndarray], edge_index: Union[Tensor, np.ndarray],
    ...
    def _to_tensor(obj, *, dtype=None) -> Optional[Tensor]:
        """Best-effort conversion to torch.Tensor. Keeps None as None."""
        if obj is None:
            return None
        if isinstance(obj, Tensor):
            return obj.to(dtype=dtype) if dtype is not None else obj
        # PyG EdgeIndex (newer versions)
        if hasattr(obj, "to_tensor"):
            t = obj.to_tensor()
            return t.to(dtype=dtype) if dtype is not None else t
        # numpy / memmap / list / tuple / other array-like
        try:
            t = torch.as_tensor(obj)
        except Exception as e:
            raise TypeError(f"Cannot convert type {type(obj)} to torch.Tensor") from e
        return t.to(dtype=dtype) if dtype is not None else t

    # ---- Force everything into torch.Tensor (joblib may pass numpy/memmap/list) ----
    x = _to_tensor(x)
    edge_index = _to_tensor(edge_index, dtype=torch.long)   # indices should be long
    edge_attr = _to_tensor(edge_attr) if edge_attr is not None else None
    y = _to_tensor(y) if y is not None else None

    # Safety: ensure x is Tensor for construct_features (it uses .index_select)
    if not isinstance(x, Tensor):
        raise TypeError(f"x must be a torch.Tensor after conversion, got {type(x)}")

    # Ensure edge_index shape is [2, E]
    if edge_index.dim() != 2:
        raise ValueError(f"edge_index must be 2D tensor [2, E], got shape={tuple(edge_index.shape)}")
    ...
    if init_edges and simplex_tree.dimension() >= 1:
        if edge_attr is None:
            ...
            ex = dict()
            for e, edge in enumerate(edge_index.cpu().numpy().T):
